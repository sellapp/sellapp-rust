...
We'll start by printing one product's name. The compiler has enough to think about; your first request can be simple.
...
**Use a local path dependency for now.** This SDK is pre-release. The intended crates.io package is `sellapp`, but publication and namespace ownership are not yet verified. The repository is private, so cloning requires access.

The crate uses Rust edition 2024. Use a current stable Rust toolchain; a minimum supported Rust version has not yet been established for the full dependency graph. Clone and build the SDK:

```sh
git clone https://github.com/sellapp/sellapp-rust.git
cd sellapp-rust
cargo build
```

To use the SDK in a separate application, add this to its `Cargo.toml`. The path points Cargo at your checkout; adjust it if the SDK lives elsewhere. Tokio runs the async example below.

```toml
[dependencies]
sellapp = { path = "../sellapp-rust" }
...
Your API key identifies you; your store slug selects the store. Give the key the `listing` ability, which grants access to the catalog. For a storefront at `example-store.sell.app`, the slug is `example-store`.
...
[Report an SDK issue](https://github.com/sellapp/sellapp-rust/issues) if you have repository access.
