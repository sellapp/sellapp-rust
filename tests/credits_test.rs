...
#[tokio::test]
async fn credits_record_round_trip() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path_matcher("/v2/credit-transactions"))
        .respond_with(ResponseTemplate::new(200).set_body_string(include_str!(
            "fixtures/sdk_record_credit_transaction_response_value_200_application_json.json"
        )))
        .expect(1)
        .mount(&server)
        .await;
    ...
    let _ = client
        .credits()
        .record(sellapp::resources::credits::RecordParams::new(
            serde_json::from_str(include_str!(
                "fixtures/sdk_record_credit_transaction_request_application_json.json"
            ))
            .expect("parse fixture for SdkRecordCreditTransactionRequestApplicationJson"),
        ))
        .await
...
#[tokio::test]
async fn credits_record_unauthorized() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn credits_record_not_found() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn credits_record_rate_limited() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn credits_record_server_error() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn credits_record_bad_request() {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn credits_record_unprocessable() {
    let server = MockServer::start().await;
