// This file is auto-generated by oagen. Do not edit.

#[allow(unused_imports)]
use super::*;
#[allow(unused_imports)]
use crate::enums::*;
use serde::{Deserialize, Serialize};
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(remote = "Self")]
pub struct ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey {
    pub id: i64,
    pub key: String,
    pub limit: i64,
    pub active: i64,
    pub status: String,
    pub expires_at: Option<String>,
    pub is_expired: bool,
    pub store_id: i64,
    pub order_id: Option<i64>,
    pub invoice_id: Option<i64>,
    pub product_id: i64,
    pub variant_id: i64,
    pub customer_id: i64,
    pub subscription_id: Option<i64>,
    pub created_at: String,
    pub updated_at: String,
    pub instances_count: i64,
    /// Fields returned by a newer API version that are not yet known to this SDK.
    #[serde(default, flatten)]
    pub additional_properties: std::collections::HashMap<String, serde_json::Value>,
}
impl Serialize for ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        Self::serialize(self, serializer)
    }
}
impl<'de> Deserialize<'de>
    for ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey
{
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let mut value = serde_json::Value::deserialize(deserializer)?;
        crate::runtime_schema::model(
            &value,
            "ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey",
            false,
        )
        .map_err(serde::de::Error::custom)?;
        crate::runtime_schema::retain_model_fields(
            &mut value,
            "ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey",
        );
        Self::deserialize(value).map_err(serde::de::Error::custom)
    }
}
impl ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey {
    /// Validate the serialized request, including schema constraints and compatibility rules.
    pub fn validate_request(&self) -> Result<(), crate::error::Error> {
        let value = serde_json::to_value(self)
            .map_err(|e| crate::error::Error::Serialization(e.to_string()))?;
        crate::runtime_schema::model(
            &value,
            "ValidateLicenseKeyResponseValue200ApplicationJsonPropertyDataPropertyLicenseKey",
            true,
        )
    }
}
